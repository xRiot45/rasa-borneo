...
interface TransactionRangeFilterProps<TData, TValue> {
    column: Column<TData, TValue>;
    title?: string;
}

export function TransactionRangeFacetedFilter<TData, TValue>({ column, title = 'Tanggal' }: TransactionRangeFilterProps<TData, TValue>) {
        ...
        <Popover>
            <PopoverTrigger>
                <Button variant="outline" className="w-[350px] justify-start text-left font-normal">
                    ...
                    {range?.min || range?.max ? `Transaksi: ${range?.min ?? '0'} - ${range?.max ?? '∞'}` : title}
                </Button>
            </PopoverTrigger>

...
export function DataTableRowActions({ row }: { row: Row<MenuItem> }) {
    ...
    const handleRestoreData = (id: number) => {
        router.patch(
            route('merchant.menu-items.restore', { id }),
            {},
            {
                onSuccess: () => {
                    toast.success('Success', {
    ...
    const handleForceDelete = (id: number) => {
        router.delete(route('merchant.menu-items.forceDelete', { id }), {
            onSuccess: () => {
                toast.success('Success', {
        ...
        <>
            <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-[260px]">
                    <Link href={route('merchant.menu-items.edit', { menuItem: row.original.slug })} className="cursor-pointer">
